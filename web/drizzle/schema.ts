...
foreignKey({ columns: [table.talosId], foreignColumns: [tlsTalos.id], name: "tls_patrons_talosId_fkey" }).onUpdate("cascade").onDelete("cascade"),
]);
...
foreignKey({ columns: [table.talosId], foreignColumns: [tlsTalos.id], name: "tls_activities_talosId_fkey" }).onUpdate("cascade").onDelete("cascade"),
]);
...
index("tls_commerce_jobs_lease_idx").using("btree", table.leasedBy.asc().nullsLast().op("text_ops"), table.leaseExpiresAt.asc().nullsLast().op("timestamp_ops")).where(sql`"status" = 'pending'`),
...
foreignKey({ columns: [table.talosId], foreignColumns: [tlsTalos.id], name: "tls_commerce_jobs_talosId_fkey" }).onUpdate("cascade").onDelete("cascade"),
]);
...
foreignKey({ columns: [table.talosId], foreignColumns: [tlsTalos.id], name: "tls_commerce_services_talosId_fkey" }).onUpdate("cascade").onDelete("cascade"),
]);
...
foreignKey({ columns: [table.talosId], foreignColumns: [tlsTalos.id], name: "tls_approvals_talosId_fkey" }).onUpdate("cascade").onDelete("cascade"),
]);
...
foreignKey({ columns: [table.talosId], foreignColumns: [tlsTalos.id], name: "tls_revenues_talosId_fkey" }).onUpdate("cascade").onDelete("cascade"),
]);
...
foreignKey({ columns: [table.talosId], foreignColumns: [tlsTalos.id], name: "tls_playbooks_talosId_fkey" }).onUpdate("cascade").onDelete("cascade"),
]);
...
foreignKey({ columns: [table.playbookId], foreignColumns: [tlsPlaybooks.id], name: "tls_playbook_purchases_playbookId_fkey" }).onUpdate("cascade").onDelete("cascade"),
]);
...
// 0016: durable background-job execution queue (PR #323)
export const tlsJobs = pgTable("tls_jobs", {
id: text().primaryKey().notNull(),
queue: text().notNull(),
...
leaseOwner: text(),
leaseExpiresAt: timestamp({ precision: 3, mode: 'string' }),
heartbeatAt: timestamp({ precision: 3, mode: 'string' }),
attempts: integer().default(0).notNull(),
maxAttempts: integer().default(8).notNull(),
retryClass: text().default('transient').notNull(),
...
index("tls_jobs_status_runAt_idx").using("btree", table.status.asc().nullsLast().op("text_ops"), table.runAt.asc().nullsLast().op("timestamp_ops")),
index("tls_jobs_queue_status_idx").using("btree", table.queue.asc().nullsLast().op("text_ops"), table.status.asc().nullsLast().op("text_ops")),
index("tls_jobs_leaseExpiresAt_idx").using("btree", table.leaseExpiresAt.asc().nullsLast().op("timestamp_ops")),
uniqueIndex("tls_jobs_queue_idempotencyKey_unique").using("btree", table.queue.asc().nullsLast().op("text_ops"), table.idempotencyKey.asc().nullsLast().op("text_ops")).where(sql`"idempotencyKey" IS NOT NULL`),
]);
